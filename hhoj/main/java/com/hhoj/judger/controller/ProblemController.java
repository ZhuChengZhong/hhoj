...
public class ProblemController {
	...
	public ModelAndView list(Problem problem,@PathVariable(value="page") Integer page) {
		ModelAndView mav=new ModelAndView();
		int count=problemService.findCount(problem);
		List<Problem>list=problemService.findProblems(problem);
		mav.addObject("problemList", list);
		if(page==null) {
			page=1;
		}
		PageBean pageBean=new PageBean(10,page, count);
		String url="problem/list";
		String pagination=PageUtil.getPagination(url, pageBean);
		mav.addObject("pagination", pagination);
		mav.addObject("mainPage", "/WEB-INF/jsp/problem/list.jsp");
